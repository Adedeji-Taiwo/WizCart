...
import { MdLogin, MdOutlineLogout, MdOutlineSpaceDashboard } from 'react-icons/md';
import { FaAngleDown } from 'react-icons/fa';
import { VscClose } from 'react-icons/vsc'
import userIcon from '../../../../assets/userProfile/user-icon.png';
    ...
    <HeaderMain>
        ...
        <HeaderSearchContainer focus={isMenuOpen}>
            ...
            <SearchBtn focus={isMenuOpen}>
                {window.innerWidth < 1024 ? (
                    <img src={search} alt="search" title={t("Search")} />
                ) : (<AiOutlineSearch />)}
            </SearchBtn>
        </HeaderSearchContainer>
        ...
        <MobileAvatar closeDropdown={dropDown} >
                {userInfo ? (
                    <div className='avatarContainer '>
                        <p className="avatar" onClick={() => setDropDown(false)}>
                            {userInfo.name ? userInfo.name.split(' ').map(item => item[0]).join().replace(',', '') : userInfo.email.substring(0, 2)}
                        </p>
                        <p className='avatar-icon'><FaAngleDown /></p>
                    </div>
                ):(
                    <div className='avatarContainer'>
                        <img src={userIcon} alt="icon" title={t("Profile")} onClick={() => setDropDown(false)}  className='avatar' />
                        <p className='avatar-icon'><FaAngleDown /></p>
                    </div>
                )}
                <div className='profile-dropdown'>
                    <div>
                        {userInfo ? (
                             <p>{userInfo.name ? userInfo.name : userInfo.email.split('@')[0]}</p>
                        ):(
                            <>
                        ...
                        {userInfo ? (
                             ...
                             <p className='icon-activity'>
                              ...
                              {userInfo.role === 'shopper' ? (
                                 <span onClick={() => navigate(`/${encodeURI(t('user'))}/${encodeURI(t('history'))}`)}>{t("Dashboard")}</span>
                              ) : (
